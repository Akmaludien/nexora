...
export async function POST(request:Request){
  ...
  const subject=`login:${parsed.data.email.toLowerCase()}:${request.headers.get("x-forwarded-for")??"local"}`;const limit=await incrementRateLimit({subject,action:"login",limit:5,windowSeconds:300});if(!limit.allowed){console.warn(JSON.stringify({event:"login_rate_limited",email:parsed.data.email,ip:request.headers.get("x-forwarded-for")??"local"}));return NextResponse.json({error:"Too many login attempts."},{status:429,headers:{"Retry-After":String(limit.retryAfter)}});}
  const user=await verifyCredentials(parsed.data.email,parsed.data.password);if(!user){console.warn(JSON.stringify({event:"login_failed",email:parsed.data.email,reason:"invalid_credentials",ip:request.headers.get("x-forwarded-for")??"local"}));return NextResponse.json({error:"Invalid email or password."},{status:401});}
